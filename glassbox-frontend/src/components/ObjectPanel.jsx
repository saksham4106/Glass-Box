...
import {ObjectHeapPanel} from "./ObjectHeapPanel.jsx";
 ...
 */
export default function ObjectPanel({ frame, steps, stepIndex, changed, trackingConfig, isGraph }) {
  if (!frame) {
    return (
  ...
  if(!isGraph){
    ...
    return (
        <div className="panel object-panel" style={{width:'100%', height:'100%'}}>
          <center><h2 className="panel-title">{frame.id}</h2></center>
          {items.length === 0 ? (
              <p className="empty-hint">No collections in this frame.</p>
          ) : (
              <ObjectHeapPanel step={steps[stepIndex]} frame={frame}/>
          )}
        </div>
    )
  }
